...
#ifndef BOOST_PROTO_UNPACK_EXPR_HPP_INCLUDED
#define BOOST_PROTO_UNPACK_EXPR_HPP_INCLUDED
...
namespace boost
{
    namespace proto
    {
        ...
        inline constexpr auto unpack_expr(Tag tag, Seq && seq)
        BOOST_PROTO_AUTO_RETURN(
            proto::domains::unpack_expr<deduce_domain>(
                static_cast<Tag &&>(tag)
              , static_cast<Seq &&>(seq)
            )
        )
        ...
        namespace functional
        {
            struct unpack_expr
            {
                ...
                constexpr auto operator()(Tag tag, Seq && seq) const
                BOOST_PROTO_AUTO_RETURN(
                    proto::unpack_expr(static_cast<Tag &&>(tag), static_cast<Seq &&>(seq))
                )
            };
        }
        ...
        namespace result_of
        {
            ...
            struct unpack_expr
            {
                using type = decltype(proto::unpack_expr(Tag(), std::declval<Seq>()));
            };
        }
    }
}
